...
export const migrations: Migration[] = [
  ...
  },
  {
    id: '0010_multiplayer',
    sql: `
      -- How to join a game that is already running, per title. Mirrors the
      -- launch and save rules: an operator writes it once, every client uses it.
      CREATE TABLE game_multiplayer_rules (
        id TEXT PRIMARY KEY,
        game_id TEXT NOT NULL REFERENCES games(id) ON DELETE CASCADE,
        -- Argument template, e.g. "+connect {address}:{port}". Substituted and
        -- passed to the process as separate arguments, never through a shell.
        join_args TEXT NOT NULL,
        -- Used when the host's client cannot report one.
        default_port INTEGER,
        -- Optional extra arguments for the host, to start a listen server.
        host_args TEXT,
        note TEXT,
        created_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ','now'))
      );
      CREATE UNIQUE INDEX game_multiplayer_rules_game_idx ON game_multiplayer_rules(game_id);

      -- What each player is currently in, and whether anyone can join it.
      -- One row per player: a person is in one game at a time.
      ...
      -- A connect string handed to one person, for a short while.
      ...
      CREATE INDEX game_invites_to_idx ON game_invites(to_user_id, expires_at);
    `,
  },
];
