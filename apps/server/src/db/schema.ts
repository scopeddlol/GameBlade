...
/** How to join a running game, written once per title by an administrator. */
export const gameMultiplayerRules = sqliteTable(
  'game_multiplayer_rules',
  {
    id: text('id').primaryKey(),
    gameId: text('game_id')
      .notNull()
      .references(() => games.id, { onDelete: 'cascade' }),
    joinArgs: text('join_args').notNull(),
    defaultPort: integer('default_port'),
    hostArgs: text('host_args'),
    note: text('note'),
    createdAt: text('created_at').notNull().default(now),
  },
  (t) => [uniqueIndex('game_multiplayer_rules_game_idx').on(t.gameId)],
);

/** What each player is in right now, and whether it can be joined. */
export const gameSessions = sqliteTable(
  'game_sessions',
  {
    ...
    port: integer('port'),
    joinable: integer('joinable', { mode: 'boolean' }).notNull().default(false),
    startedAt: text('started_at').notNull().default(now),
    lastSeenAt: text('last_seen_at').notNull().default(now),
  },
  (t) => [index('game_sessions_game_idx').on(t.gameId)],
);

/** A connect string handed to one friend, valid for a short while. */
export const gameInvites = sqliteTable(
  ...
  (t) => [index('game_invites_to_idx').on(t.toUserId, t.expiresAt)],
);
